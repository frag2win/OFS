...
def execute(
    ...
    if not force:
        ...
        if index.has_changes():
            print("⚠️  WARNING: You have uncommitted changes in the staging area")
    ...
    files_to_restore = commit.get('files', [])
    ...
    for file_entry in files_to_restore:
        ...
        if action == 'deleted':
            ...
            # Restore file (added or modified)
            try:
    ...
    try:
        ...
        # Add all files from commit to index (except deleted ones)
        for file_entry in files_to_restore:
            if file_entry.get('action') != 'deleted':
                index.add(
                    file_entry['path'],
    ...
    print(f"✓ Checked out to commit {commit_id} \"{message}\"")
    ...
    if force:
        index = Index(repo.index_file)
        if index.has_changes():
            print(f"  ⚠️  Uncommitted changes were discarded")
